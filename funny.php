...
error_reporting(0);
 // 当前数据库里面有106条数据
 $total = query("SELECT count(*) AS total FROM posts WHERE category_id = '3'");
//  print_r($total);
//  exit();
    ...
    <div class="content">
      ...
      <div class="page-action">
          <ul class="pagination pagination-sm pull-right">
            <li>
              <a href="/funny.php?page=<?php echo $prevPage?>">上一页</a>
            </li>
            <?php foreach($pages as $key => $val){?>
              <?php if($pageCurrent == $val){ ?>
              <li class="active">
                <a href="/funny.php?page=<?php echo $val?>"><?php echo $val?></a>
              </li>
              <?php }else { ?>
              <li>
                <a href="/funny.php?page=<?php echo $val?>"><?php echo $val?></a>
          ...
          <?php }?>
            <li>
              <a href="/funny.php?page=<?php echo $nextPage?>">下一页</a>
            </li> 
          </ul>
        </div>
    </div>
