...
if ($action == 'deleteAll') {
    ...
    exit;
} else if ($action == 'delete' && isset($_GET['comments_id'])) {
    //删除
    $pid = $_GET['comments_id'];
...
} else if ($action == 'status' && isset($_GET['comments_id'])) {
    $cid = $_GET['comments_id'];
    $row = query("SELECT * FROM comments WHERE id = {$cid}");
    if (empty($row)) {
        $msg = '评论不存在';
    } else {
        $data = $row[0];
            ...
            <?php foreach ($com_lists as $key => $vals) { ?>
                <tr class="danger">
                    ...
                    <td class="text-center">
                        <a href="/admin/comments.php?action=status&comments_id=<?php echo $vals['id'] ?>"
                           class="btn btn-info btn-xs"><?php echo $vals['status'] == 'approved' ? '驳回' : '批准';?></a>
                        <a href="/admin/comments.php?action=delete&comments_id=<?php echo $vals['id'] ?>"
